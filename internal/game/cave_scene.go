...
func (c *CaveScene) Update(g *Game) error {
	...
	if planktonCount < 120 {
		for i := 0; i < 2; i++ {
			rx := g.camera.Pos.X + rand.Float64()*float64(config.ScreenWidth)
			ry := g.camera.Pos.Y + rand.Float64()*float64(config.ScreenHeight)
			if ry >= 0 {
	...
	// Update cave entities using the entity.Runtime interface adapter
	entityRuntime := &entityRuntimeAdapter{g: g}
	...
	// Apply deferred entity mutations
	...
	var drag = speedProps.Drag // Drag resistance in water
	...
	if swimming && speed > 2.0 && rand.Float64() < 0.12 {
		g.Particles = append(g.Particles, particle.NewBubbleParticle(p.Pos.X+p.Width/2.0, p.Pos.Y+p.Height/2.0))
	}
...
func (c *CaveScene) Draw(g *Game, finalScreen *ebiten.Image) {
	...
	if shader.LightShader != nil && !c.IsShallow && !g.DebugDisableLightShader {
		...
		entranceX := float32(float64(len(c.CaveGrid)/2*config.TileSize) + config.TileSize/2.0 - g.camera.Pos.X)
	...
	for _, ent := range c.Entities {
		if siphon, ok := ent.(*entity.BrimstoneSiphon); ok && siphon.IsActive() {
	...
	if shader.WaterDisplacementShader != nil && !g.DebugDisableWaterShader {
