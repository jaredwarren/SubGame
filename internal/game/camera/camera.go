...
func (c *Camera) CenterOn(targetX, targetY, targetWidth, targetHeight float64) {
	c.Pos.X = targetX + targetWidth/2.0 - float64(config.ScreenWidth)/2.0
	c.Pos.Y = targetY + targetHeight/2.0 - float64(config.ScreenHeight)/2.0
}
...
func (c *Camera) Track(targetX, targetY, targetWidth, targetHeight, lerpRate float64) {
	destX := targetX + targetWidth/2.0 - float64(config.ScreenWidth)/2.0
	destY := targetY + targetHeight/2.0 - float64(config.ScreenHeight)/2.0
