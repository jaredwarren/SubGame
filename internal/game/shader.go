...
func Fragment(position vec4, texCoord vec2, color vec4) vec4 {
	...
	if dist < LightRadius && dist > 0.0 {
		...
		if dotVal >= outerDot {
			...
			angularFade := (dotVal - outerDot) / (minDot - outerDot)
...
func Fragment(dstPos vec4, srcPos vec2, color vec4) vec4 {
	...
	// Clamp to destination image bounds (1280x720) to avoid rendering artifacts at borders
	distortedCoords = clamp(distortedCoords, vec2(0.5), vec2(1279.5, 719.5))
